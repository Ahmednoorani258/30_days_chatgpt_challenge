...
dic.fromkeys([1, 2, 3])  # Creates a new dictionary with keys from the iterable and values set to `None`
...
dic.setdefault(1)  # Returns the value of key `1` if it exists, otherwise sets it to `None` and returns it
dic.update({1: 2})  # Updates the dictionary with the key-value pairs from another dictionary or iterable
...
st.discard(1)  # Removes the element `1` from the set if it exists (does not raise an error if it doesn't exist)
...
st.remove(1)  # Removes the element `1` from the set (raises an error if the element doesn't exist)

...
st.write("This app performs sentiment analysis using pre-trained model from Hugging Face Transformer.")
...
if "classifier" in st.session_state:
    if task == "Sentiment Analysis":
        text = st.text_input("Enter text for sentiment analysis:", "I love using Hugging Face!")
        if st.button("Analyze Sentiment"):
            result = st.session_state.classifier(text)
            st.write(f"Sentiment: **{result[0]['label']}** (Confidence: {result[0]['score']:.4f})")
else:
...
if st.button("Save Prediction") and "result" in locals():
    if task == "Sentiment Analysis":
        st.session_state.predictions.append({"Task": task, "Text": text, "Sentiment": result[0]['label']})

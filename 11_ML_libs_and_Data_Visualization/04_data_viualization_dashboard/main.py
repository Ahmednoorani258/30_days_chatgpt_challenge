...
st.markdown("Explore the Titanic dataset by filtering by passenger class and gender, and visualizing different aspects of the data.")
...
for vis in vis_options:
    ...
    if vis == "Survived vs Age":
        ...
        ax.set_title(f"Survived vs Age for Class {pclass}" + (f" and Gender {gender}" if gender != "All" else ""))
        ...
        ax.set_title(f"Age Distribution for Class {pclass}" + (f" and Gender {gender}" if gender != "All" else ""))
        ...
        ax.set_title(f"Fare Distribution for Class {pclass}" + (f" and Gender {gender}" if gender != "All" else ""))
        ax.set_xlabel("Fare")
        ax.set_ylabel("Count")
        st.pyplot(fig)
        ...
        ax.set_title(f"Survival Rate for Class {pclass}" + (f" and Gender {gender}" if gender != "All" else ""))
        ax.set_xlabel("Survived")
